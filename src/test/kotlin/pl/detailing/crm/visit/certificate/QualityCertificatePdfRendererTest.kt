...
class QualityCertificatePdfRendererTest {
    ...
    private fun data(
        ...
        thankYou = listOf("Dziękujemy za zaufanie.", "Poniżej zestawienie prac."),
        services = services,
        usedProducts = used,
    ...
    fun `nie pokazuje sekcji zaleceń, gdy nic nie polecono`() {
        ...
        assertTrue(text.contains("UŻYTE PRODUKTY"), "sekcja użytych produktów powinna zostać")
    }
    ...
    fun `mówi wprost, gdy nie wskazano usług ani produktów`() {
        ...
        assertTrue(text.contains("ZAKRES WYKONANYCH USŁUG"), "belka sekcji ma zostać")
        assertTrue(text.contains("Nie wskazano usług"), "pusta lista musi się tłumaczyć: $text")
    }
