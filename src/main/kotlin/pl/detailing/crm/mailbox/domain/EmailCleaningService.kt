...
/**
 * Turns a raw e-mail body into the text the UI renders and the LLM reads.
 *
 * Quoted history is the reason threads get expensive: by the third message a client's
 * reply carries the whole conversation again, which both bloats the LLM context and
 * makes the thread unreadable in the UI. Removing it is deterministic work, so it is
 * done once at ingest and stored, rather than paid for on every prompt.
 */
@Service
