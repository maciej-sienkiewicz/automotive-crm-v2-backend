...
/**
 * Global exception handler for the Trends module.
 */
@RestControllerAdvice(basePackages = ["com.example.demo.trends"])
class TrendsExceptionHandler {
    ...
    fun handleValidation(ex: TrendsValidationException): ResponseEntity<ErrorResponse> {
        ...
        return ResponseEntity.badRequest().body(
            ErrorResponse("VALIDATION_ERROR", ex.message, HttpStatus.BAD_REQUEST.value())
        )
    }
    ...
    fun handleAuth(ex: DataForSeoAuthException): ResponseEntity<ErrorResponse> {
        ...
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(
            ErrorResponse("AUTH_ERROR", ex.message, ex.statusCode)
        )
    }
    ...
    fun handleRateLimit(ex: TrendsRateLimitException): ResponseEntity<ErrorResponse> {
        log.warn("DataForSEO rate limit hit: {}", ex.message)
        return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body(
            ErrorResponse("RATE_LIMIT", ex.message, ex.statusCode)
        )
    }
    ...
    fun handleApiError(ex: DataForSeoApiException): ResponseEntity<ErrorResponse> {
        ...
        return ResponseEntity.status(HttpStatus.BAD_GATEWAY).body(
            ErrorResponse("API_ERROR", ex.message, ex.statusCode)
        )
    }
    ...
    fun handleGeneric(ex: Exception): ResponseEntity<ErrorResponse> {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(
            ErrorResponse("INTERNAL_ERROR", ex.message)
        )
    }
}
